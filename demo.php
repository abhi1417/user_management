...
echo $query  = "SELECT * FROM user_leave $WHERE";
$result = $conn->query($query);
...
<?php include("includes/footer.php");?>
<script type='text/javascript'>
onsubmit="return formValidator(myform)"
    ...
    function formValidator(){
        ...
        for (var i = 0; i < required_fields.length; i++) {
            if(required_fields[i].value == ""){
                ...
                // required_fields[i].parentElement.parentElement.children[2].innerHTML=error_message;
                document.getElementsByClassName(required_fields[i].id)[0].innerHTML = error_message;;
                
                
            } else {
                required_fields[i].className = required_fields[i].className.replace(" required_error", '')
                //required_fields[i].parentElement.parentElement.children[2].innerHTML="";
                document.getElementsByClassName(required_fields[i].id)[0].innerHTML = "";
                not_emp++;
            }
            
            submit_success = 0;
        }
        if(not_emp == required_fields.length){
            return true;
        } else {
            return false;
